...
// const Pokemon_Secreto = Pokemon_Total[Math.floor(Math.random() * Pokemon_Total.length)].toUpperCase();

// let Pokemon_Total = [];

let Pokemon_Secreto = "";
let Largo_De_Las_Casillas = 0;
...
async function inicializarJuego() {
  try {
    ...
    const Pokemon_Total = data.results.map((pokemon) =>
      pokemon.name.toUpperCase()
    );

    // Adquiere un Pokemon secreto.
    Pokemon_Secreto =
      Pokemon_Total[Math.floor(Math.random() * Pokemon_Total.length)];
    Largo_De_Las_Casillas = Pokemon_Secreto.length;

    // Invoca la función para iniciar el juego.
    iniciarTablero();
  } catch (error) {
...
function iniciarTablero() {
  ...
  switch (dia) {
    ...
    case 4:
      url =
        "https://assets.pokemon.com//assets/cms2/img/misc/virtual-backgrounds/masters/cave.jpg";
    ...
    case 6:
      url =
        "https://assets.pokemon.com//assets/cms2/img/misc/virtual-backgrounds/masters/canyon.jpg";
  ...
  for (let contador = 0; contador < 5; contador++) {
    ...
    */
      bloque_de_grilla.addEventListener("keydown", (e) => {
        ...
        if (/^[A-ZÑ]$/.test(letra)) {
          // Las expresiones regulares sirven para verificar patrones en cadenas de texto.
          //Devuelve booleano True si lo ingresado en la casilla
          ...
          if (
            siguiente &&
            siguiente.classList.contains("bloque_de_la_grilla")
          ) {
            siguiente.focus();
          }
          ...
          if (bloque_de_grilla.textContent != "") {
            //Si el cáracter escrito es distinto a
            ...
            // En caso de ya estar en nada, cambia el focus a la anterior casilla.
            ...
            if (
              anterior &&
              anterior.classList.contains("bloque_de_la_grilla")
            ) {
          ...
          const filaCompleta = Array.from(casillas).every((casilla) => {
            // Revisa que todas las casillas tengan una letra.
          ...
          if (filaCompleta) {
            ...
            const aciertoCompleto = Array.from(casillas).every(
              (casilla, index) => {
            ...
            if (aciertoCompleto) {
              setTimeout(
                () => alert("¡Felicidades! Has adivinado el Pokémon."),
                100
              );
            } else if (fila.classList.contains("grilla_final")) {
              setTimeout(
                () => alert(`¡Perdiste! El Pokémon era: ${Pokemon_Secreto}`),
                100
              );
            }
          } else {
          ...
          if (siguienteFila) {
            const siguienteCasilla = siguienteFila.querySelector(
              ".bloque_de_la_grilla"
            );
            if (siguienteCasilla) {
